<template>
    <div class="main-page">
        <div v-if="isList">
            <div class="page-top">
                <div class="records-top-close" @click="close">
                    <img :src="require('../../assets/login/back.png')" />
                    <span>Back</span>
                </div>
                <div class="record-tools">
                    <div :class="{'active': gameDateType == 1}" style="width: 35%" @click="changeGameRecords(1)">{{ GLOBAL.lanLocal['today'] }}</div>
                        <div :class="{'active': gameDateType == 2}" style="width: 35%" @click="changeGameRecords(2)">{{ GLOBAL.lanLocal['yesterday'] }}</div>
                        <div :class="{'active': gameDateType == 3}" style="width: 30%">
                        <input type="date" style="width: 100px;" v-model="gameRecordDate" @change="setGameRecords" placeholder="select"/>
                    </div>
                </div>
            ...
            <div class="records-table"  @scroll="handleScroll" :style="{'height': (innerHeight-pageTopHeight-10) + 'px'}">
                <div class="table-item" :class="{'game-multi': index == 0 || index % 2 == 0}" v-for="(item, index) in gameRecord">
                    ...
                    <div class="desc" style="width: 30%;">
                        {{item.DayTime}}
                        <img @click="toDetail(item.ServerID)" :src="require('../../assets/me/game_arrow.png')"/>
                    </div>
                </div>
        ...
        <div v-if="!isList">
            ...
            <div class="detail-table" @scroll="handleDetailScroll" :style="{'height': (innerHeight-pageDetailTopHeight-10) + 'px'}">
                <div class="table-item" :class="{'game-multi': index == 0 || index % 2 == 0}" v-for="(item, index) in gameRecordDetail">
