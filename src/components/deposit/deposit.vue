<template>
    <div class="main-page">
        ...
        <div class="content" v-if="type == 1">
            <div>
                <div class="recharge-box">
                    <div v-for="(item, index) in moneyData" :key="index + 1" class="recharge-item" @click="genMoney(item.PayMoney)"
                        :class="{'active': money == item.PayMoney, 'recharge-item-mid': (((index - 1 == 0) || ((index-1)%3 == 0)))}">{{GLOBAL.currencySymbol}}{{ item.PayMoney }}
        ...
        <div class="content" v-if="type == 2">
            <div v-if="isRechage">
                <div class="withdraw-title">{{ GLOBAL.lanLocal['candraw'] }}:{{GLOBAL.currencySymbol}}&nbsp;{{ userAmount }}</div>
                <div class="recharge-box">
                    <div v-for="(item, index) in withdrawAmount" :key="index + 1" class="recharge-item" @click="genAmount(item)"
                    :class="{'active': amount == item, 'recharge-item-mid': (((index - 1 == 0) || ((index-1)%3 == 0)))}">{{GLOBAL.currencySymbol}}{{ item }}<img
                            :src="require('../../assets/amount/select-icon.png')" /></div>
                </div>
                <div class="s-box">
                    <div class="link">
                        <div class="login-info-item">
                ...
                <div class="s-box">
                    <div class="link">
                        <div class="login-info-item">
                            <div class="v-box2">
                                <input class="v-input2" type="number" v-model="amount" @input="genRealAmount" :disabled="withdrawConfig.IsManual != 1"
                ...
                <div class="desc">
                    {{GLOBAL.lanLocal['fee']}}: {{withdrawConfig.Fee}}%, {{GLOBAL.lanLocal['realamount']}}:{{GLOBAL.currencySymbol}} {{realAmount}}
                </div>
            ...
            <div v-else><div class="content" v-html="withdrawConfig.NoChargeDescript"></div></div>
        </div>
        <div class="card-masking" v-show="showCardModal" @click="changeCardModal">
            <div class="card-modal" @click.stop="func1()">
                ...
                <div class="card-footer">
                    ...
                    <input class="cancel-btn" type="button" @click="changeCardModal" :value="GLOBAL.lanLocal['paycancel']" />
                </div>
            </div>
        </div>
...
.recharge-box {
    ...
    .recharge-item {
        ...
        box-shadow: 0 0.02rem 0.2rem rgba(0,0,0,.3);
        text-align: center;
        margin-bottom: 5px;
        position: relative;
        font-size: 20px;
